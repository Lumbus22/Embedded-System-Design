...
void USART2_read_string(char *, int);
...
void GreetUser(void){
	...
	while(yesOrNo == 'y'){
	char output[50];
	USART2_write_string("What is your name?\r\n");
	USART2_read_string(output, 50);
	USART2_write_string("Hello, ");
	USART2_write_string(output);
	USART2_write_string("! Have a great day!\r\n");
	USART2_write_string("Would you like to enter a new name? (y/n)");
	yesOrNo = USART2_read_char();

	//Flush the leftover \r that come from pressing enter
    HAL_Delay(5);
    while(USART2->SR & (1 << 5)){
        (void)USART2->DR;
    }
...
void USART2_init(void){
	...
	*/
	...
	USART2->BRR = 0x1117; //The system clock config is running he syustem at 84MHz. so BRR = 42000000 / 9600 = 4375 = 0x1117
...
//This simple function will read the Data Register whenever the RXEN line goes high.
char USART2_read_char(void) {
	while (!(USART2 -> SR & (1 << 5)));
...
//This function will recursively read characters to form a string. There must be a max amount of characters so that the function knows when to stop reading.
